  ...
  this.setState(
    {
      ...
      amts:String(data[1]).split(','),
      vnames:String(data[2]).split(',')
    }
  )
}
loggedIn(event){
...
render (){
  ...
  _.each(this.state.items,(value, index)=> {
     TableRows.push(
       < tr>
       <h2>
       <td><span style={{marginLeft:35,fontSize:20}}>{counter++}</span></td>

       <td><span style={{marginLeft:35,fontSize:20}}>{ETHEREUM_CLIENT.toAscii(this.state.items[index])}</span></td>
       <td><span style={{marginLeft:50,fontSize:20}}>{this.state.amts[index]}</span></td>
       <td><span style={{marginLeft:55,fontSize:20}}>{ETHEREUM_CLIENT.toAscii(this.state.vnames[index])}</span></td>

       <span><input type="button" style={{marginLeft:55,fontSize:20}} onClick={this.display} value=" CONFIRM " /></span>
       </h2>
       </tr>
...
<center>      <table style={{marginTop:80}}>
        <thead>
          ...
          <h1>
           ...
           <th><span style={{marginLeft:30,fontSize:20}}>QuoteId</span></th>
              <th><span style={{marginLeft:30,fontSize:20}}>Item</span></th>
              <th><span style={{marginLeft:30,fontSize:20}}>Price</span></th>
              <th><span style={{marginLeft:30,fontSize:20}}>Vendor</span></th>
     ...
     <h1><table>
     <button  onClick={this.loggedIn.bind(this)}> * * Navigate Blocks!! * * </button>
     </table></h1>
